...
def get_parser():  # pylint: disable=too-many-statements,too-many-locals
    ...
    parser = argparse.ArgumentParser(
        'helper.py', description='oss-fuzz helpers')
    ...
    reproduce_parser.add_argument(
        'testcase_path', help='path of local testcase')
...
def build_image_impl(project, cache=True, pull=False, architecture='x86_64'):
    ...
    logger.info('\033[31mHACK: \033[0m')
...
def coverage(args):  # pylint: disable=too-many-branches
    ...
    if args.corpus_dir:
        ...
        run_args.extend(['-v', '%s:/corpus/%s' %
                        (corpus_dir, args.fuzz_target)])
    else:
...
def introspector(args):
    ...
    shutil.copytree(os.path.join(args.project.out,
                    "inspector"), introspector_dst)
